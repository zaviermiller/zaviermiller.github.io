  ...
  data: () => ({
    ...
    archived: [
      ...
      // Zephyr
      {
        ...
        links: [
          {
            to: "https://github.com/zaviermiller/zephyr",
            icon: "mdi-github",
            name: "Github",
          },
        ],
...
<template>
  <v-row class="justify-center pt-8">
    <v-col cols="12" class="text-center pb-8">
      <h1
        style="font-family: 'Chakra Petch' !important; color: #ccc"
        ...
        <v-col cols="auto" v-for="project in featured" :key="project.name">
          <v-hover v-slot="{ hover }">
            <v-card
              color="background"
              elevation="0"
              style="transition: 0.7s ease-out; cursor: pointer; color: #aaa"
              class="pa-4 featured shifted-down"
              ...
              <v-responsive :aspect-ratio="1">
                <v-card-text
                  :style="`height: 300px; width:300px; white-space: pre; font-family: 'JetBrains Mono' !important; text=align: center;${
                    ...
                    hover
                      ? 'color: ' + project.color + '; text-shadow: 0 0 4px;'
                      : ''
                  }`"
                  >{{ thumbLogos[project.slug] }}</v-card-text
                >
                <v-card-text class="justify-center d-flex align-center">
                  <p
                    :style="`font-family: 'JetBrains Mono'; font-size: 18px; transition: .3s ease-out; ${
                      hover
                        ? 'text-shadow: 0 0 4px; color: ' + project.color + ';'
                        : ''
                    }`"
                  >
                    {{ project.name }}
    ...
    <v-col cols="12" class="text-center mt-12">
      <h1
        style="font-family: 'Chakra Petch' !important; color: #ccc; opacity: 0"
        class="text-h3 text-md-h2 mt-6 mb-12"
        id="archiveTitle"
      >
        Other Projects
      </h1>
