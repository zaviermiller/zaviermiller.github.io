...
<template>
  <div>
    <v-app-bar
      ...
      absolute
      color="rgba(0,0,0,0)"
      :class="`px-md-12 mx-md-12 ${drawer ? 'mt-4' : 'my-4'}`"
      id="appbar"
    >
      <v-toolbar-title
        style="font-family: 'Aleo' !important; cursor: pointer"
        class="text-h4 mb-n1"
        @click="$router.push('/')"
        >Z</v-toolbar-title
      >
      ...
      <template v-if="$vuetify.breakpoint.mdAndUp">
        <v-btn
          ...
          class="appbar-items"
          :ripple="false"
          style="font-family: 'JetBrains Mono'; text-transform: none !important"
          exact-active-class="underlined"
    ...
    <v-expand-transition v-if="$vuetify.breakpoint.smAndDown">
      ...
      >
        <DashSeperator
          width="80%"
          style="margin-left: -40%; position: relative; left: 50%"
        />
        <v-list-item>
          <v-btn
            text
