<template>
  <v-col cols="12" class="text-center py-2">
    <NuxtLink
      class="text-subtitle-2 text-md-subtitle-1 font-weight-regular primary--text link"
      style="
    ...
    >
      <span :class="`${color}--text`" style="text-transform: capitalize"
        >[{{ post.category }}]</span
      >
      {{ post.title }}
    </NuxtLink>
...
export default {
  ...
  computed: {
    ...
    dots() {
      return ".".repeat(
        TOTAL_LEN - this.post.title.length - this.post.category.length - 3,
      )
    },
  },
}
